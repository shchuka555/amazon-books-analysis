...
In this project, I collected data from Amazon using Python, cleaned and transformed the data gathered into a more useable form by SQL and then visualized data with Tableau.

## Motivation

- exploring and polishing my scraping skills.
- Applying the analysis to real-world data.
- Challenging to collect data from the actual website
- Unorganized data cleaning by SQL 
- to recommend what amazon.ca should care and not about books on the market.
...
I collected roughly 45k book data from [amazon.ca]( https://www.amazon.ca/ref=nav_logo).
- collected only top100 seller books from each category and subcategory of books listed on the website


- ### Step2: collect book data with Python 

I decided to use scraps to collect data because Scrapy is much faster than other python libraries for scraping, such as Selenium and Beautiful-Soup.
I Successfully collected book ranking, book titles, URLs, and categories of books from amazon.ca. However, Scrapy often fails to connect individual book URLs and scrape data despite the URLs working.
Therefore, I decided to use selenium to collect book details data, and selenium successfully managed the process.



- The data I collected has originally roughly **45k** row and **30** columns




- ### Step3: store data in SQL database

I heard real-world data stored in databases are often untidy and unclear; this is why data analysis and data scientists sometimes spend a lot of time on data cleaning.

To make the project closer to a fundamental data analyst job, I deliberately did not clean the data I collected. However, I could have done clearing before I stored data in a database.
...
I cleaned data by removing duplicates, changing the data type of variables to a more analyzable form and splitting one variable into multiple columns.
Also, I added new columns such as estimated revenue of the book (without tax and commission fee from amazon)
...
I plotted multiple graphs and tables to show differences between kindle, audible and physical books.
